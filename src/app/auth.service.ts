...
export class AuthService implements OnInit {
  ...
  private loggedInToken = JSON.parse(
    localStorage.getItem("loggedIn") || "false"
  );
  ...
  get LoginToken() {
    return (
      localStorage.getItem("loggedInToken") || this.loggedInToken.toString()
    );
  }
  ...
  addIteminInCollection(collection: string, payload: Games) {
  ...
  async addItemToArrayInGamesPlayersDoc(id: string, payload: any) {
    this.gamesRef
      ...
      .then((res) => {
        this.gamesRef
          .doc(id)
          .update({ players: [...res.data().players, payload] });
      })
      .catch((error) => console.log(error));
  }
  async addItemToArrayInGamesWaitingDoc(id: string, payload: any) {
    this.gamesRef
      ...
      .then((res) => {
        this.gamesRef
          .doc(id)
          .update({ waiting: [...res.data().waiting, payload] });
      })
      .catch((error) => console.log(error));
  }
  ...
  async removeItemFromArrayInGamesPlayersDoc(id: string, itemToRemove: string) {
    this.gamesRef
      ...
      .toPromise()
      .then((res) => {
        const updatedPlayers = res
          .data()
          .players.filter((item) => item.uid !== itemToRemove);
  ...
  async removePlayerFromGame(id: string, itemToRemove: string, payload: any) {
    this.gamesRef
      ...
      .toPromise()
      .then((res) => {
        const updatedPlayersAfterRemove = res
          .data()
          .players.filter((item) => item.uid !== itemToRemove);
        ...
        this.gamesRef
          .doc(id)
          .update({ players: updatedPlayersAdding, waiting: updatedWaiting });
      })
      .catch((error) => console.log(error));
  }
  ...
  async removeItemFromArrayInGamesWaitingDoc(id: string, itemToRemove: string) {
    this.gamesRef
      ...
      .toPromise()
      .then((res) => {
        const updatedWaiting = res
          .data()
          .waiting.filter((item) => item.uid !== itemToRemove);
